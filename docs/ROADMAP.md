...
| Phase   | 기간  | 목표                               | 선행 조건 | 병렬 가능 |
| ------- | ----- | ---------------------------------- | --------- | --------- |
| Phase 1 | 1~2주 | 프로젝트 골격 + 전체 UI 프로토타입 | 없음      | -         |
| Phase 2 | 3주차 | 인증 시스템 + DB 기반 구축         | Phase 1   | -         |
| Phase 3 | 4주차 | 챌린지 CRUD 기능 연결              | Phase 2   | -         |
| Phase 4 | 5주차 | 참여/탈퇴 + 실시간 기능 연결       | Phase 3   | -         |
| Phase 5 | 5~6주 | 인증(Check-in) 시스템 기능 연결    | Phase 4   | -         |
| Phase 6 | 6주차 | 관리자 기능 연결 + 배포            | Phase 5   | -         |
...
- **Task 001: 프로젝트 구조 및 전체 라우트 골격 생성** `TASK-001` `todo` - 우선순위
  - Next.js App Router 기반 전체 라우트 구조 생성 (PRD 섹션 7 프로젝트 구조 기준)
  - `src/app/(public)/page.tsx` 랜딩 페이지 빈 껍데기
  - `src/app/(auth)/home/page.tsx`, `my-challenges/page.tsx`, `my-checkins/page.tsx` 빈 껍데기
  - `src/app/challenges/page.tsx`, `new/page.tsx`, `[id]/page.tsx`, `[id]/edit/page.tsx`, `[id]/checkin/page.tsx` 빈 껍데기
  - `src/app/admin/page.tsx`, `challenges/page.tsx`, `users/page.tsx`, `stats/page.tsx` 빈 껍데기
  - `src/app/auth/callback/route.ts` OAuth 콜백 라우트 빈 껍데기
  - features 디렉토리 구조 생성: `auth/`, `challenges/`, `checkins/`, `admin/` (각각 `components/`, `services/`, `types/`)
  - 관련 기능: 전체

- **Task 002: TypeScript 타입 정의 및 더미 데이터 생성** `TASK-002` `todo`
  - `src/features/auth/types/` - User 타입, AuthTypes 정의
  - `src/features/challenges/types/` - Challenge, ChallengeCategory, ChallengeStatus, ChallengeParticipant 타입 정의
  - `src/features/checkins/types/` - ChallengeCheckin 타입 정의
  - `src/features/admin/types/` - AdminTypes (통계 지표, 차트 데이터 타입) 정의
  - `src/shared/mocks/` - 더미 데이터 생성 유틸리티 (챌린지 목록, 사용자, 인증 기록, 관리자 통계 등)
  - 모든 UI 컴포넌트에서 사용할 목 데이터 상수 정의
  - 관련 기능: 전체

- **Task 003: 공통 레이아웃 및 네비게이션 UI 구현** `TASK-003` `todo`
  - `src/shared/layouts/Header.tsx` 공통 헤더 (비로그인: 챌린지 목록 + 로그인 버튼 / 로그인: 챌린지 목록 + 새 챌린지 만들기 + 사용자 드롭다운)
  - `src/shared/layouts/Footer.tsx` 공통 푸터
  - 모바일 하단 탭 네비게이션 컴포넌트 (홈/탐색/+새 챌린지/내 챌린지/프로필)
  - 로그인 상태는 더미 플래그로 전환 가능하도록 구현 (이후 실제 인증으로 교체)
  - 반응형 처리: 데스크톱 헤더 + 모바일 하단 탭
  - shadcn/ui 기본 컴포넌트 설치 (Button, Card, Badge, DropdownMenu, Dialog, Tabs, Select, Input, Textarea 등)
  - 다크모드 기본 설정 (next-themes, `.dark` 클래스 기반)
  - View Transitions API 테마 전환 애니메이션

- **Task 004: 랜딩 페이지 UI 구현** `TASK-004` `todo` `F001`
  - 랜딩 페이지 UI 구현: 히어로 섹션 (타이틀, 설명, CTA 버튼)
  - 주요 기능 소개 섹션
  - 인기 챌린지 미리보기 (더미 데이터로 ChallengeCard 3~4개 표시)
  - Google 로그인 버튼 (더미: 클릭 시 홈 페이지로 이동)
  - 반응형 디자인 적용

- **Task 005: 챌린지 목록/상세/생성/수정 페이지 UI 구현** `TASK-005` `todo` `F003` `F004` `F005` `F007` `F008`
  - `src/features/challenges/components/ChallengeCard.tsx` 카드 컴포넌트 (썸네일, 제목, 기간, 참여자 수, 상태 배지)
  - `src/app/challenges/page.tsx` 챌린지 목록 페이지 UI (더미 데이터)
  - 카테고리 필터 탭 (전체/건강-운동/학습/생활습관/기타), 기간 필터, 상태 필터, 정렬 옵션
  - 무한 스크롤 UI 구조 (`src/shared/hooks/useInfiniteScroll.ts` 빈 훅)
  - 빈 상태 화면 ("조건에 맞는 챌린지가 없습니다")
  - `src/features/challenges/components/ChallengeDetail.tsx` 상세 컴포넌트 (썸네일, 제목, 설명, 카테고리, 기간, 상태 배지, 인증 방법 안내, 참여자 수/최대 참여자 수)
  - `src/app/challenges/[id]/page.tsx` 챌린지 상세 페이지 UI (더미 데이터)
  - 운영자 전용 버튼 (수정하기, 삭제하기, 종료하기) / 참여자 전용 버튼 (참여하기, 탈퇴하기, 인증하기)
  - `src/features/challenges/components/ChallengeForm.tsx` 폼 컴포넌트
  - `src/app/challenges/new/page.tsx` 챌린지 생성 폼 UI (제목, 설명, 기간, 카테고리, 인증 방법, 최대 참여자 수, 썸네일)
  - 실시간 글자 수 표시, 필수 필드 유효성 검사 UI
  - `src/app/challenges/[id]/edit/page.tsx` 수정 페이지 UI (ChallengeForm 재사용, 수정 모드)
  - 삭제 확인 다이얼로그 UI

- **Task 006: 인증 업로드/기록 페이지 UI 구현** `TASK-006` `todo` `F011` `F012` `F013`
  - `src/features/checkins/components/CheckinUploader.tsx` 업로더 컴포넌트 UI
  - `src/app/challenges/[id]/checkin/page.tsx` 인증 업로드 페이지 UI (챌린지 제목, 인증 방법 안내, 이미지 업로드 영역, 메모 입력, 업로드 버튼)
  - 이미지 미리보기 및 교체 버튼 UI
  - 업로드 진행률 UI
  - 이미 오늘 인증 완료 시 완료 상태 화면 UI
  - `src/features/checkins/components/CheckinGrid.tsx` 전체 참여자 인증 현황 그리드 UI (더미 데이터)
  - `src/features/checkins/components/CheckinCalendar.tsx` 달력 뷰 컴포넌트 UI (더미 데이터)
  - `src/app/(auth)/my-checkins/page.tsx` 내 인증 기록 페이지 UI (참여 중 챌린지 선택 드롭다운, 월별 달력 뷰, 인증률 통계)
  - 날짜 클릭 시 인증 이미지/메모 모달 UI
  - 이전/다음 월 이동 버튼

- **Task 007: 내 챌린지/홈 페이지 UI 구현** `TASK-007` `todo` `F020`
  - `src/app/(auth)/my-challenges/page.tsx` 내 챌린지 페이지 UI (더미 데이터)
  - "운영 중" 탭: 내가 만든 챌린지 목록 (상태 배지, 참여자 수, 관리 버튼)
  - "참여 중" 탭: 내가 참여 중인 챌린지 목록 (오늘 인증 여부 배지, 전체 인증률 프로그레스바)
  - 각 챌린지 카드에 오늘 인증 완료 여부 시각적 표시
  - 빈 상태 화면 ("아직 챌린지가 없습니다. 새 챌린지를 만들어 보세요!")
  - `src/app/(auth)/home/page.tsx` 홈 페이지 UI: 참여 중인 챌린지 목록, 오늘 인증 미완료 카드 목록 (최대 5개, 인증하기 버튼), 내 전체 챌린지 인증률 요약, 인기 챌린지 추천 섹션

- **Task 008: 관리자 페이지 UI 구현** `TASK-008` `todo` `F016` `F017` `F018` `F019`
  - `src/shared/layouts/AdminLayout.tsx` 관리자 레이아웃 (사이드바 + 메인 콘텐츠)
  - `src/features/admin/components/AdminSidebar.tsx` 사이드바 (대시보드, 챌린지 관리, 사용자 관리, 통계)
  - `src/app/admin/page.tsx` 관리자 대시보드 UI (더미 데이터): 핵심 지표 카드 4개 (MAU, 전체 챌린지 수, 오늘 인증 수, 신규 가입자), 최근 7일 인증 수 BarChart, 카테고리별 비율 PieChart, 최근 챌린지/사용자 목록
  - `src/app/admin/challenges/page.tsx` 챌린지 관리 페이지 UI (더미 데이터): 테이블, 상태/카테고리 필터, 상태 변경 드롭다운, 삭제 버튼, 페이지네이션
  - `src/features/admin/components/UserTable.tsx` 사용자 테이블 컴포넌트
  - `src/app/admin/users/page.tsx` 사용자 관리 페이지 UI (더미 데이터): 테이블, 검색, 활성화/비활성화 토글, 페이지네이션
  - `src/app/admin/stats/page.tsx` 통계 페이지 UI (더미 데이터): 기간 선택 탭, 신규 가입자 LineChart, 챌린지 생성 BarChart, 인증 업로드 BarChart, 카테고리별 참여자 BarChart (Recharts)

- **Task 009: 로딩 스켈레톤, 빈 상태, 에러 UI 구현** `TASK-009` `todo`
  - 로딩 스켈레톤 UI: 챌린지 목록, 챌린지 상세, 인증 현황, 관리자 테이블
  - 빈 상태(Empty State) UI 전체 페이지 적용 (챌린지 목록, 내 챌린지, 내 인증 기록 등)
  - 에러 바운더리 및 전역 에러 처리 (`error.tsx`, `not-found.tsx`)
  - 인증 업로드 실패 시 재시도 UI
  - 네트워크 오류, 타임아웃 등 공통 에러 처리 UI
  - 로그인 페이지 UI (returnUrl 파라미터 지원)
...
- **Task 011: Google OAuth 로그인 및 인증 미들웨어 구현** `TASK-011` `todo` `F001` `F002`
  - Supabase Auth Google OAuth Provider 설정
  - `src/app/auth/callback/route.ts` OAuth 콜백 처리 구현
  - Next.js 미들웨어 설정: `(auth)` 그룹 라우트 인증 보호
  - `src/features/auth/components/GoogleLoginButton.tsx` 구현
  - `src/features/auth/components/AuthButton.tsx` 로그인/로그아웃 상태 분기 구현
  - 로그인 페이지 구현 (returnUrl 파라미터 지원) - Phase 1에서 만든 UI에 실제 인증 로직 연결
  - 로그아웃 기능 구현 (세션 종료 후 랜딩 페이지 리디렉션)
  - 공통 헤더/모바일 탭의 더미 로그인 상태를 실제 인증 상태로 교체
  - Next.js 미들웨어 확장: `/admin/*` 라우트에 대해 role='admin' 체크, 미인가 시 홈 페이지 리디렉션
